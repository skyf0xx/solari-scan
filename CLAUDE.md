...
A CLI that clones a GitHub repo/PR into an isolated Solari code sandbox,
executes its install and build steps (never the test suite), and reports
observed runtime behavior — unexpected outbound network destinations and
unexpected filesystem writes outside the repo directory — as facts, not
a safe/unsafe verdict. Built for a security-literate audience evaluating
untrusted PR code before merge, without running that code on their own
machine.
